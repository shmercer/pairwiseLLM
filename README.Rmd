...
Each provider has its own `build_*_batch_requests()`, `run_*_batch_pipeline()`, and `parse_*_batch_output()` functions. A unified batch front-end (e.g., `llm_submit_pairs_batch()`) is planned to mirror `submit_llm_pairs()`.
...
A unified batch front-end is planned (e.g., `llm_submit_pairs_batch()` and `llm_download_batch_results()`) to mirror the live unified API.
