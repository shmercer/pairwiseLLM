# Focused Phase 6 collection, not a full-package coverage claim.
# Rscript --vanilla data-raw/warm-start/check-issue-259-phase6.R /tmp/issue259-phase6
...
local({
  ...
  if (length(arguments) == 1L) {
    ...
    writeLines(c(
      ...
      '  filter = "^(0031|010[0-9]|011[0-8]|310[0-278]|510[015]|910[0-245])-",',
  ...
  lines <- lines[grepl("(^|/)R/warm_start_.*[.]R$", lines$filename), ]
  ...
  if (any(colSums(results[, c("failed", "error", "warning")]) > 0)) {
    stop("Focused checks failed; saved results and coverage retain diagnostic evidence.")
  }
})
