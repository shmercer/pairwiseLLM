...
4.  Use the existing Phase B linking workflow with the reference as the
    fixed hub.
5.  Apply the saved calibration to the completed accepted Phase B
    result.
...
variant and compatible settings. To collect the links, use
[`adaptive_rank()`](https://shmercer.github.io/pairwiseLLM/reference/adaptive_rank.md)
as shown below with your combined sample table, compatible `btl_config`,
local or configured judge, and planned comparison budget.
`linking_samples` includes unique `ID` and `global_item_id`, `text`, and
`set_id` values 1 and 2. This upstream call is never run during
documentation builds. Keep production quality gates and resume the
session until completion; exhausting a budget alone does not establish
precision.
...
For several targets, use the existing `link_multi_spoke` workflow. Phase
B hard-locks hub locations and retains its shared judge parameters,
D-optimal selection, probes, and stopping. Rubric labels are not
required during linking.
...
predicts the original reference items. Every spoke needs an accepted
refit and committed active linking evidence. The accepted public
`theta_link_eap` field retains its historical name: Phase B locations
are MAP estimates with Laplace/Hessian uncertainty, not MCMC posterior
means. Validated per-spoke provenance remains available, including
accepted-state reuse.
