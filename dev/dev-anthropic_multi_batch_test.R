## --------------------------------------------------------------------
## Example live test for the Anthropic batch API using pairwiseLLM
##
...
## submits all batches to the Anthropic batch API, polls until completion,
...
## this script you must set the `ANTHROPIC_API_KEY` environment variable
## before invoking R.
##
...
pairs <- example_writing_samples %>%
  make_pairs() %>%
  sample_pairs(n_pairs = 10, seed = 1234) %>%
  randomize_pair_order(seed = 5678)
...
out_dir <- file.path("dev-output", "anthropic_multi_batch_test")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

cat("Starting Anthropic multi‑batch test...\n")
...
cat("ANTHROPIC_API_KEY length:", nchar(Sys.getenv("ANTHROPIC_API_KEY")), "(characters)\n")
cat("Job details:\n")
print(
  do.call(
    rbind,
...
res <- llm_resume_multi_batches(
  ...
  combined_csv_path  = "anthropic_combined_results.csv"
)
