# dev/dev-openai-template-ab-test-3.R
#
...
out_dir <- "dev-output/openai-template-ab-test-3"
...
template_T2 <- "
Act as an impartial scoring algorithm for student writing. Your objective is to compare two texts solely based on {TRAIT_NAME}.

Definition of {TRAIT_NAME}:
{TRAIT_DESCRIPTION}

---
TEXT BLOCK A (labeled SAMPLE_1):
{SAMPLE_1}

---
TEXT BLOCK B (labeled SAMPLE_2):
{SAMPLE_2}

---

EVALUATION STEPS (Perform these silently):
1. Analyze SAMPLE_1 against the definition of {TRAIT_NAME}.
2. Analyze SAMPLE_2 against the definition of {TRAIT_NAME}.
3. Compare the two directly. Ask yourself: 'Does the first sample outperform the second, or does the second outperform the first?'
4. Explicitly disregard the sample labels (1 vs 2) during comparison to ensure no positional bias.
5. Select the winner.

FINAL REQUIREMENT:
Output the result using the exact format below. Do not provide an explanation.

<BETTER_SAMPLE>SAMPLE_1</BETTER_SAMPLE>
<BETTER_SAMPLE>SAMPLE_2</BETTER_SAMPLE>

(Select only one of the above lines).
"
