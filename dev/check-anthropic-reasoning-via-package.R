# dev/check-anthropic-reasoning-via-package.R

check_anthropic_reasoning_via_package <- function(
  ...
  for (m in models) {
    if (verbose) {
      cat("\n==================================================\n")
      cat("[Package reasoning check ", idx, "] model = ", m, "\n", sep = "")
    }
    ...
    res <- tryCatch(
      pairwiseLLM::anthropic_compare_pair_live(
        ...
        reasoning         = "enabled",
        # Important: extended thinking requires temp = 1
        temperature       = 1,
        # And valid thinking budget / max_tokens according to your function
        max_tokens             = 2048,
