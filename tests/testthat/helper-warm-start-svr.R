warm_svr_fixture <- function(schema = "writing_features_v1") {
  ...
  list(x = x, theta = theta, schema = schema,
    plan = make_warm_start_cv_plan(x$item_id, theta, "phase5", seed = 259L))
}

warm_svr_fit <- function(f = warm_svr_fixture(), ...) {
  fit_warm_start_model(f$x$item_id, f$theta, "phase5", features = f$x,
    schema = f$schema, cv_plan = f$plan, engine = "svr_rbf", ...)
}
