test_that("btl_mcmc_state_new creates a valid initial state", {
  samples <- tibble::tibble(
    ID = c("A", "B", "C"),
    text = c("alpha", "beta", "charlie")
  )

  state <- pairwiseLLM:::btl_mcmc_state_new(samples, config = list())

  expect_silent(pairwiseLLM:::validate_btl_mcmc_state(state))
  expect_s3_class(state, "adaptive_state")
  expect_equal(length(state$unordered_count), 3L)
  expect_true(all(state$unordered_count == 0L))
})

test_that("btl_mcmc_record_judgment_exposure updates counts consistently", {
  samples <- tibble::tibble(
    ID = c("A", "B"),
    text = c("alpha", "beta")
  )
  ...
  state2 <- pairwiseLLM:::btl_mcmc_record_judgment_exposure(state, "A", "B")
...
test_that("btl_mcmc_state_save/load roundtrip preserves class and fields", {
  samples <- tibble::tibble(
    ID = c("A", "B"),
    text = c("alpha", "beta")
  )
  ...
  pairwiseLLM:::btl_mcmc_state_save(state, path)
  loaded <- pairwiseLLM:::btl_mcmc_state_load(path)
...
test_that("validate_btl_mcmc_state rejects non-adaptive_state and bad results_seen", {
  expect_error(pairwiseLLM:::validate_btl_mcmc_state(list()), "adaptive_state")
  ...
  expect_error(pairwiseLLM:::validate_btl_mcmc_state(state), "results_seen")

  state <- pairwiseLLM:::btl_mcmc_state_new(samples, config = list())
  state$results_seen <- c(TRUE, FALSE)
  expect_error(pairwiseLLM:::validate_btl_mcmc_state(state), "named")
})
