test_that("saved adaptive sessions reject malformed objects and missing artifacts", {
  ...
  expect_silent(save_adaptive_session(state, session))
  saveRDS(list(), file.path(session, "state.rds"))
  expect_error(validate_session_dir(session), "adaptive_state")
  expect_silent(save_adaptive_session(state, session, overwrite = TRUE))
  ...
  expect_silent(loaded <- load_adaptive_session(session))
  expect_identical(loaded$meta$now_fn(), state$meta$now_fn())
...
test_that("stop boundary backfill counts committed evidence and refuses further replay at the cap", {
  ...
  session <- withr::local_tempdir()
  expect_silent(save_adaptive_session(fixed, session))
  expect_silent(resumed <- load_adaptive_session(session))
