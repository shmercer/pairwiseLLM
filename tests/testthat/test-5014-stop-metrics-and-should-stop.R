...
test_that("adaptive global stopping cannot reconstruct a removed Phase B posterior", {
  state <- task10_link_state(2L)
  expect_error(compute_stop_metrics(state, config = list()),
    class = "pairwiseLLM_link_selector_unvalidated")
})

test_that("E1 conditional uncertainty and E2/E3 full uncertainty remain distinct", {
  for (id in .adaptive_link_estimation_mode_levels()) {
    result <- fit_link(link_contract_input(id, 2L))
    metrics <- .link_orchestration_view(result)
    expect_identical(metrics$covariance, result$uncertainty$covariance)
    expect_true(result$diagnostics$fit_valid)
    expect_true(all(is.finite(result$items$theta_link_sd)))
    if (id == "fixed_shape_offset") {
      expect_match(result$diagnostics$uncertainty_scope, "conditional")
    } else {
      expect_false(grepl("conditional", result$diagnostics$uncertainty_scope))
    }
  }
})
