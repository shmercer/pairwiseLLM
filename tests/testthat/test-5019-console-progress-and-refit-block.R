...
test_that("adaptive_rank_run_live prints linking-specific refit summary lines", {
  ...
  expect_true(any(grepl("probe_brier=.*pass=", combined)))
  expect_true(any(grepl("probe_pred_rmse_lagged=.*pass=", combined)))
  expect_true(any(grepl("theta_global_rmse_lagged=.*pass=", combined)))
