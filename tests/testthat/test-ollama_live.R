...
testthat::test_that("submit_ollama_pairs_live catches errors thrown by inner function", {
  ...
  testthat::with_mocked_bindings(
    ...
    {
      res <- submit_ollama_pairs_live(
        pairs, model="m", trait_name="t", trait_description="d", verbose=FALSE
      )

      testthat::expect_equal(nrow(res), 1L)
      testthat::expect_true(is.na(res$status_code))
      testthat::expect_match(res$error_message, "Error during Ollama comparison: Inner crash detected")
    }
  )
})
