test_that("lossless public storage preserves portable models and missing-feature preprocessing", {
  ...
  path <- file.path(tempdir(), ".", basename(tempfile(fileext = ".rds")))
  ...
  saved <- save_warm_start_model(model, path)
  canonical_path <- normalizePath(path, winslash = "/", mustWork = TRUE)
  expect_identical(saved, canonical_path)
  ...
  expect_identical(save_warm_start_model(model, path, overwrite = TRUE), canonical_path)
