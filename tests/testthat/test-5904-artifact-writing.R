testthat::test_that("artifact writing writes expected files", {
  ...
  for (path in c(config_path, round_log_path, batch_log_path, draws_path)) {
    testthat::expect_true(file.exists(path))
    testthat::expect_true(file.info(path)$size > 0)
  }
  testthat::expect_false(file.exists(file.path(output_dir, "item_summary.rds")))
