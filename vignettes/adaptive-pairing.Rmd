...
vignette: >
  %\VignetteIndexEntry{Adaptive Pairing and Ranking in pairwiseLLM}
...
1. **Select a pair**
   A candidate pair is chosen based on uncertainty, coverage needs, and structural constraints.

2. **Obtain a judgment**
   The pair is evaluated (typically by an LLM), producing a binary outcome.

3. **Online update**
   The online state is updated immediately.

4. **Periodic Bayesian refit**
   After enough new comparisons, all accumulated data are refit using Bayesian BTL to produce posterior estimates, diagnostics, and stopping metrics.
...
Adaptive runs periodically fit a Bayesian BTL model and therefore require
CmdStan. Before the first run, install CmdStanR, verify the compiler toolchain,
and install CmdStan as described in the
...
We show how to:
- load example data,
- select a writing trait to evaluate,
- define a prompt template,
- run adaptive ranking with a live LLM judge,
- and extract logs for further analysis.
...
* **Model**: `gpt-5.1`
* **Service tier**: `"flex"` (lower-cost, slower OpenAI Flex processing; capacity may be unavailable)
...
### Linking workflows with `adaptive_rank()`

`adaptive_rank()` is also the high-level entry point for linking runs:

* `run_mode = "link_one_spoke"` for one spoke set,
* `run_mode = "link_multi_spoke"` for multi-spoke runs.

In linking mode:

* Phase B is hub↔spoke by default.
* Phase B only starts after valid Phase A artifacts are available.
* Multi-spoke execution is concurrent and hub parameters are hard-locked;
  these are enforced contract properties rather than user controls.
* Returned logs include `link_stage_log` in addition to `step_log`, `round_log`, and `item_log`.

```{r eval=FALSE}
# Two-set linking with imported Phase A artifacts
res_link <- adaptive_rank(
  data = linking_samples_df,
  id_col = "ID",
  text_col = "text",
  judge = my_judge,
  n_steps = 300L,
...
# Multi-spoke linking with persistence/resume
res_multi <- adaptive_rank(
  data = linking_samples_df,
  id_col = "ID",
  text_col = "text",
  judge = my_judge,
  n_steps = 600L,
  session_dir = "adaptive-linking-session",
  resume = TRUE,
  adaptive_config = list(
...
For a complete technical specification of the adaptive algorithm, including:
- pair types and quotas,
- fallback rules,
...
> Mercer, S. H. (2026). *Adaptive pairing and ranking in pairwiseLLM* [R package vignette]. 
