...
- Are often cheaper (e.g., 50% discount on OpenAI).
...
- **OpenAI** (via `run_openai_batch_pipeline()`)
- **Anthropic** (via `run_anthropic_batch_pipeline()`)
- **Gemini** (via low-level helpers such as `build_gemini_batch_requests()`)
...
- Submitting many batches (e.g., across templates, providers, and models).
- Polling batches until completion.
- Downloading and parsing results.
- Using a **batch registry CSV** to safely resume interrupted jobs.
