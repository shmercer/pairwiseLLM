...
A `pairwiseLLM_warm_predictions` tibble with ordered component
calibrated predictions, their `ensemble_mean`, and diagnostic sample
`ensemble_sd`. Component results and shared CV identity are retained as
metadata. Prediction uses stored numeric payloads without loading any
fitting backend. Text input extracts features once; precomputed features
require no Python.
