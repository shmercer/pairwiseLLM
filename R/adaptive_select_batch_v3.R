...
#' @keywords internal
#' @noRd
.adaptive_duplicate_allowed_v3 <- function(state, unordered_key, p_mean, utility, config) {
...
#' @keywords internal
#' @noRd
sample_exploration_pairs_v3 <- function(state, candidates, n_explore, config) {
  ...
  while (accepted < n_explore && attempts < max_attempts) {
    ...
    if (!is.na(count) && count >= 1L) {
      ...
      if (!.adaptive_duplicate_allowed_v3(state, unordered_key, p_mean, utility, config)) {
        next
      }
...
#' @keywords internal
#' @noRd
select_exploitation_pairs_v3 <- function(candidates_with_utility, state, n_exploit, config) {
  validate_state_v3(state, config)
  ...
  for (idx in seq_len(nrow(ordered))) {
    ...
    if (!.adaptive_duplicate_allowed_v3(
      state,
      unordered_key,
      row$p_mean[[1L]],
      row$utility[[1L]],
...
#' @keywords internal
#' @noRd
assign_order_v3 <- function(pairs, state) {
...
select_batch_v3 <- function(state, candidates_with_utility, config, seed = NULL, exploration_only = FALSE) {
  ...
  exploit <- select_exploitation_pairs_v3(
  ...
  assigned <- assign_order_v3(combined, state)
  assigned
}
