...
#' Compose multiple allocation functions
#'
#' Create an `allocation_fun` by composing multiple allocation functions.
#' Each function is called in order, and their (list) outputs are merged.
#'
#' The returned function is compatible with the `allocation_fun` argument of
#' [bt_run_core_linking()] and [bt_run_adaptive_core_linking()].
#'
#' @param ... Allocation functions (or `NULL`) to compose.
#'
#' @return A function suitable for passing as `allocation_fun`.
#'
#' @examples
#' f1 <- function(state) list(within_batch_frac = 0.2)
#' f2 <- function(state) list(core_audit_frac = 0.1)
...
allocation_compose <- function(...) {
  fns <- list(...)
  fns <- Filter(Negate(is.null), fns)
  if (length(fns) == 0L) {
    return(function(state) NULL)
  }
  if (any(!vapply(fns, is.function, logical(1)))) {
  ...
  function(state) {
    ...
    for (fn in fns) {
      ...
      if (is.null(res)) next
      if (!is.list(res)) {
        stop("Composed allocation functions must return NULL or a list.", call. = FALSE)
      }
      # Last-wins merge
      out <- utils::modifyList(out, res)
    }
