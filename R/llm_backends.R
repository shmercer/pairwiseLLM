...
#'
#' Start with `parallel = FALSE` and a small collection example. Cloud calls
#' transmit sample text to the chosen provider and may incur charges. Inspect
#' all three returned components before modeling; a successful retry can leave
#' an entry in `failed_attempts` even when the pair has a valid result.
#' Use a separate `save_path` for each study/model/trait configuration.
#' See [Getting Started](https://shmercer.github.io/pairwiseLLM/articles/getting-started.html)
#' and the [recovery guide](https://shmercer.github.io/pairwiseLLM/articles/provider-controls-and-recovery.html).
#'
