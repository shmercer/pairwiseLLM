# -------------------------------------------------------------------------
# Non-adaptive BTL MCMC state exposure helpers.
# -------------------------------------------------------------------------

#' @keywords internal
#' @noRd
btl_mcmc_record_presentation <- function(state, A_id, B_id) {
...
#' @keywords internal
#' @noRd
btl_mcmc_rollback_presentation <- function(state, A_id, B_id) {
  if (!A_id %in% state$ids || !B_id %in% state$ids) {
    rlang::abort("`A_id` and `B_id` must exist in `state$ids`.")
  }
...
#' @keywords internal
#' @noRd
btl_mcmc_record_judgment_exposure <- function(state, A_id, B_id) {
  if (!A_id %in% state$ids || !B_id %in% state$ids) {
    rlang::abort("`A_id` and `B_id` must exist in `state$ids`.")
  }
...
#' @keywords internal
#' @noRd
btl_mcmc_record_exposure <- function(state, A_id, B_id) {
  state <- btl_mcmc_record_presentation(state, A_id, B_id)
  btl_mcmc_record_judgment_exposure(state, A_id, B_id)
}
